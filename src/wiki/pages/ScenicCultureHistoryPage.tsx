// src/wiki/pages/ScenicCultureHistoryPage.tsx
...
export default function ScenicCultureHistoryPage() {
  const nav = useNavigate()
  const [category, setCategory] = useState<string | undefined>(undefined)
  const categories = useMemo(() => listCategories('culture-history'), [])
  const entries = useMemo(
    () => listWikiEntries('culture-history', category),
    [category]
  )
  ...
  useEffect(() => {
    ...
    loadWikiModuleBySlug('culture-history', selected.category, selected.slug).then(setMod).catch(() => setMod(null))
  }, [selected])
